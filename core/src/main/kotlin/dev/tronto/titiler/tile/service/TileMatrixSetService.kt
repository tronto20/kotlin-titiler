...
class TileMatrixSetService(
    private val tileMatrixSetFactory: TileMatrixSetFactory,
) : TileMatrixSetUseCase {
    override suspend fun tileMatrixSets(): List<TileMatrixSet> {
        return tileMatrixSetFactory.list().toList()
    }

    override suspend fun tileMatrixSet(options: OptionProvider<TileMatrixSetOption>): TileMatrixSet {
        return tileMatrixSetFactory.fromId(options.get(ArgumentType<TileMatrixSetOption>()).tileMatrixSetId)
    }
}
